...
public class FabricMapChunkCache extends MapChunkCache {
    ...
    public static void init() {
        if (!init) {
            Field[] f = ServerChunkManager.class.getDeclaredFields();
...
//    		    }
    ...
    private Object getNBTValue(NbtElement v) {
        Object val = null;
        switch (v.getType()) {
            case 1: // Byte
                val = ((NbtByte) v).byteValue();
                break;
            case 2: // Short
                val = ((NbtShort) v).shortValue();
                break;
            case 3: // Int
                val = ((NbtInt) v).intValue();
                break;
            case 4: // Long
                val = ((NbtLong) v).longValue();
                break;
            case 5: // Float
                val = ((NbtFloat) v).floatValue();
                break;
            case 6: // Double
                val = ((NbtDouble) v).doubleValue();
                break;
            case 7: // Byte[]
                val = ((NbtByteArray) v).getByteArray();
                break;
            case 8: // String
                val = ((NbtString) v).asString();
                break;
            case 9: // List
                NbtList tl = (NbtList) v;
                ArrayList<Object> vlist = new ArrayList<Object>();
                int type = tl.getHeldType();
                for (int i = 0; i < tl.size(); i++) {
                    switch (type) {
                        ...
                        case 8:
                            ...
                            break;
                        case 10:
                            NbtCompound tc = tl.getCompound(i);
            ...
            case 10: // Map
                ...
                for (Object t : tc.getKeys()) {
                    String st = (String) t;
                    NbtElement tg = tc.get(st);
                    vmap.put(st, getNBTValue(tg));
                }
                val = vmap;
                break;
            case 11: // Int[]
                val = ((NbtIntArray) v).getIntArray();
                break;
        }
        return val;
    }
    ...
    private boolean tryChunkCache(DynmapChunk chunk, boolean vis) {
        ...
        SnapshotCache.SnapshotRec ssr = DynmapPlugin.plugin.sscache.getSnapshot(dw.getName(), chunk.x, chunk.z, blockdata, biome, biomeraw, highesty);
        if (ssr != null) {
            ss = ssr.ss;
            if (!vis) {
    ...
    private SnapshotCache.SnapshotRec prepChunkSnapshot(DynmapChunk chunk, NbtCompound nbt) throws ChunkSnapshot.StateListException {
        ChunkSnapshot ss = new ChunkSnapshot(nbt, dw.worldheight);
        DynIntHashMap tileData = new DynIntHashMap();

        NbtList tiles = nbt.getList("TileEntities", 10);
        if (tiles == null) tiles = new NbtList();
        /* Get tile entity data */
        List<Object> vals = new ArrayList<Object>();
        for (int tid = 0; tid < tiles.size(); tid++) {
            NbtCompound tc = tiles.getCompound(tid);
            int tx = tc.getInt("x");
            int ty = tc.getInt("y");
            int tz = tc.getInt("z");
            int cx = tx & 0xF;
            ...
            if (te_fields != null) {
                ...
                for (String id : te_fields) {
                    NbtElement v = tc.get(id);  /* Get field */
                    if (v != null) {
    ...
    public int getLoadedChunks() {
        ...
        while (iter.hasNext()) {
            ...
            else if (cps.isChunkLoaded(chunk.x, chunk.z)) {
                ChunkSnapshot ss;
